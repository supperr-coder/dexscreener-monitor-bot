...
def _clean_database_url(raw: str) -> str:
    ...
    - ensure scheme starts with postgresql+asyncpg://
    - remove libpq-only params (sslmode, channel_binding) that break asyncpg
    """
    if not raw:
    ...
    # parse and strip bad query params
    parts = urlsplit(raw)
...
async def run_sql_file(path: str):
    """
    asyncpg cannot execute multiple statements in one prepared stmt.
    Split on semicolons and run each with exec_driver_sql.
    """
